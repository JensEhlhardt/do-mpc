...
def template_model(system_type='SX', nD_Super=40, modelUsage=0):
        ...
        kSuper = 1.5  # [W/m²] heat loss coefficient
        ...
        T_sat = CP.PropsSI("T", "Q", 0, "P", P, "Water")-273.15 a # [°C] saturation temperature
        ...
        # set the inputs
        Qdot_S = model.set_variable(var_type='_u', var_name='Qdot_E', shape=(1, 1))  # [%] heating power percentage evaporator
        ...
        ## define RHS ##################################################################################################
        # predefine the RHS
        da_E = SX.sym('da_E',  nD_Super, 1)
        dT_FE = SX.sym('dT_FE', nD_Super, 1)
        dT_WE = SX.sym('dT_WE', nD_Super, 1)
        dT_PE = SX.sym('dT_PE', nD_Super, 1)

        # calculate the heating power from MV
        Qdot_S = Qdot_S * 3600 / 100  # [W]
        ...
        for i in range(0, nD_Super):
            ...
            if i != 0 and i != (nD_Super-1): # 2-order CDF
        ...
        dT_WE[0] = D_W * 2 * (T_WE[1] - T_WE[0]) / dz_E**2 / (cpW * rhoW ) \
                   ...
                   - kSuper * A_r_W * (T_WE[0] - T_Env) / (cpW * rhoW * A_z_W * dz_E)
