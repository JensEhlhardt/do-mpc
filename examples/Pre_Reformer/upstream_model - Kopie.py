   ...
   # v_F    = Function('v_F', [aE, aEN], [v_L * ((aE+aEN)*(1-S_L)+S_L)])
    ...
    for i in range(0, nD_Evap):
        if i != 0:
            ...
            # volume fractions
            da_EN[i] = -v_L * (a_EN[i] - a_EN[i-1])/dz_E + (mDotC(a_EN[i], a_E[i], T_FE[i]) - mDotE_iN)/rhoL
    ...
    da_EN[0] = -v_L * (a_EN[0] - a_InN)/dz_E + (mDotC(a_EN[0], a_E[0], T_FE[0]) - mDotE(a_EN[0], T_FE[0], T_sat))/rhoLN
    ...
    S_C = 1
    s_CC = S_C * (rhoVN * 0.84 * 18)/(rhoV * 12)
    model.set_expression('S_C', fmin(a_V / (a_VN[-1]*s_CC + 1e-5),5))
